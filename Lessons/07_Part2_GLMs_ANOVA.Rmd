...
```{r}
#does mean biomass differ between groups?
#Wrangle the data
...
shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_046"]) #normal distr, p-value = 0.4021
...
shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_057"]) #normal distr,  p-value = 0.1343
...
qqnorm(Litter.Totals$dryMass); qqline(Litter.Totals$dryMass) #add line
...
Litter.Totals.groups <- HSD.test(Litter.Totals.anova, "plotID", group = TRUE) #group lvls w/ same mean
Litter.Totals.groups #shows possible treatment groups, groups w/ same letter are not significantly different (have same mean)
...
summary(Litter.anova.2way) #reject null, means are diff

# Format as lm
Litter.anova.2way2 <- lm(data = Litter, dryMass ~ functionalGroup + nlcdClass)
summary(Litter.anova.2way2) #means are diff
...
```{r}
# Format as aov
Litter.anova.2way3 <- aov(data = Litter, dryMass ~ functionalGroup * nlcdClass) #use * to see interaction
