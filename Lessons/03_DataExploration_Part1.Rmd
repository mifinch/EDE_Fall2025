...
> Answer: It will run the packages and load it into memory. 
...
read.csv(
  file="/home/guest/EDE_Fall2025/Data/Raw/USGS_Site02085000_Flow_Raw.csv",
...
# What happens if we don't assign a name to our imported dataset? The output opens in the console, rather than storing in a dataframe
read.csv(here("./Data/Raw/USGS_Site02085000_Flow_Raw.csv"),stringsAsFactors = TRUE)

read.csv(here("Data/Raw/USGS_Site02085000_Flow_Raw.csv"))
...
> ANSWER: Data were collected from the U.S. Geological Survey's Water Data website. The website, tool used,  selections and naming conventions are listed in the metadata, which helps allow for reproducible data analysis.
...
class(USGS.flow.data) #class is data.frame
colnames(USGS.flow.data) #return column names
...
head(USGS.flow.data) #first 6
head(USGS.flow.data, 10) #change default of 6 points to 10
tail(USGS.flow.data, 5)
USGS.flow.data[30000:30005, c(3, 8, 14)] #slice of rows

class(USGS.flow.data$datetime) #factor -> we need in date/time object)
class(USGS.flow.data$discharge.mean) #numeric
class(USGS.flow.data$gage.height.mean) #numeric

summary(USGS.flow.data)  #could point to column only with $, provides summary stats

```

What happened to blank cells in the spreadsheet when they were imported into R?
> Answer: They are listed as NA for numeric, blank for characters/factors.
...
```{r Finding NAs}
#use summary stats to find NAs
summary(USGS.flow.data$discharge.mean) # 5108 rows w/ NA
summary(USGS.flow.data$gage.height.mean) # 24870 rows w/ NA
```
...
> Answer: It would depend on the goal and context of analysis. It is often better to remove only when necessary, as "no data can" also be informative (eg if showing non-response).

```{r Removing NAs}
USGS.flow.data.complete <- na.omit(USGS.flow.data) #takes out all missing data
dim(USGS.flow.data)
dim(USGS.flow.data.complete) #goes from 33690 to 5342 observations


mean(USGS.flow.data.complete$discharge.mean) #now we can calc mean since we removed missing data
...
#site number brought in as integer, R removes leading 0 -> change into character/factor
...
```{r}
USGS.flow.data$site_no <- as.factor(USGS.flow.data$site_no) #coerce integer to factor
str(USGS.flow.data)
```
...
USGS.flow.data2  <- read.csv(
  file = here("./Data/Raw/USGS_Site02085000_Flow_Raw.csv"),
  stringsAsFactors = TRUE,
  colClasses = c('site_no' = 'factor') #import column as factor
  ) 
```
...
today <- Sys.Date()
format(today, format = "%B") #September
format(today, format = "%b") #Sep
format(today, format = "%A") #Thursday
format(today, format = "%a") #Thu

USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%m/%d/%y") 
USGS.flow.data$datetime[1] # "2027-10-01"??
```
...
# change back to correct formatting
