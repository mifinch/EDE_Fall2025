...
```{r read in turtle data}
#Create the data source url
data_source2 <- paste0('https://services.arcgis.com/QVENGdaPbd4LUkLV/arcgis/rest',
                      '/services/survey123_3d7e213f47a94f1b829aa7cf54eab095_results',
                      '/FeatureServer/0/query?outFields=*&where=1%3D1&f=geojson')

#Read in to a spatial dataframe
sea_turtles <- st_read(data_source2)
...
turtle_map <- leaflet(sea_turtles) %>% 
  addTiles() %>% 
  addMarkers(
    #clusterOptions = markerClusterOptions(), #create cluster map
    popup = ~species
  )
...
```{r eval=FALSE}
##Works on desktop version, not in the Container
