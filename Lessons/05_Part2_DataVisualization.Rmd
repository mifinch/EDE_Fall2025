...
O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_bw() #black & white, removes gray
print(O3plot2) 

O3plot3 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_light() #gray lines, white background
print(O3plot3)
...
mytheme <- theme_classic(base_size = 14) + #increase font size
  theme(axis.text = element_text(color = "black"), #axis text to black
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) + #add horizontal (dashed) line at 50
  geom_hline(yintercept = 100, lty = 2) + #add horizontal line at 100
  geom_point(alpha = 0.5, size = 1.5) + #transparency/size of points
  geom_text(x = as.Date("2020-01-01"), y = 45, label = "good", hjust = 1, fontface = "bold") +   #add text, location on x and y axis, hjust = 1 means left align 
  ...
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")),#specify date range
    date_breaks = "2 months", date_labels = "%b %y") + #change breaks, format of date, reorder
  ylab(expression("O"[3]* " AQI Value")) + #change y label (expression used for subscript)
...
NvsP2 <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth, shape = lakename)) +
  geom_point(alpha = 0.7, size = 2.5) + #transparency/size of points
  labs(color = "Depth", shape = "Lake Name") + # change your legend labels here
  scale_shape_manual(values = c(15, 17)) + #choose shapes
...
#use rel_height to adjust each plot height
