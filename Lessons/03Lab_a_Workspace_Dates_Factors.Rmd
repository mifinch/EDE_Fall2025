...
ntl_ltr <- read.csv(
  ...
  stringsAsFactors = TRUE  #chr stored as factor
  ) 

# 2.
library(tidyverse);library(here)

#read in all csv files starting w/ "EPAair_O3" into single data frame
folder <- here ('Data', 'Raw')
files <- dir (folder, pattern = "EPAair_O3*.*", full.names = TRUE)

EPAData <- files %>% #apply 2 files
  map(read.csv) %>% #read csv
  bind_rows() #combine by rows
...
format(today, format = "%Y") #2025
format(today, format = "%y") #25
...
class(sites$dow) #factor

#List the unique values in the column
unique(sites$dow)

#Summary - is it meaningful?
summary(sites$dow) #shows how many times each day of the week appears in the dataset
