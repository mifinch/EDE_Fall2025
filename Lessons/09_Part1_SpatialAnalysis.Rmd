...
EPAair.sites <- EPAair %>% 
  ...
  summarize(
    mean_Ozone = mean(Ozone, na.rm=T), #What does "na.rm=T" do? Is it necessary? na.rm ignores missing data so we can calculate the mean
    mean_PM25 = mean(PM2.5, na.rm=T),
    count_all = n(),                   #Adds a record count column
    count_Ozone = sum(!is.na(Ozone)),  #Counts the rows w/Ozone data (all values that are not null)
...
EPAair.sf <- EPAair.sites %>% 
  st_as_sf( 
    coords = c('mean_Lng','mean_Lat'), #Longitude is x (East-west), Latitude is y (vertical)
    crs=4326
    )
#st_as_sf takes input and converts it into a spatial dataframe
...
#cex = size, this makes higher pm25 values larger in size
...
sfc_GraingerHall <- st_sfc(sfg_GraingerHall,crs=4326) #includes (Geometry, Coordinate reference system)
