...
class(PeterPaul.chem.nutrients$month) #integer
...
#Alter to include missing months that were dropped
ggplot(
  PeterPaul.chem.nutrients,
  aes(x=factor(month,levels = 1:12, labels = month.abb)))+
...
#Apply a ggtheme - wall street journal
surface_temp_1999_plot + 
  theme_wsj() + 
  scale_color_wsj()
...
#Add a title
surface_temp_1999_plot + 
  ggtitle("Surface temperature, 1999") + #add title
  labs(color='Lake') + #chage legend name
  theme_wsj()
```
...
PeterPaul.chem.nutrients %>% 
  ...
  geom_point() +
  facet_wrap(facets=vars(lakename), nrow=1,ncol=2)+ #facet by lake in 2 rows
...
PeterPaul.chem.nutrients %>%
    ...
    geom_point(size = 0.5) + #transparency/size of points
...
EPAair %>% 
  ggplot(aes(
    x= PM2.5, y=Ozone, color = mean_Lat), alpha = 0.5) + 
  geom_point() + 
  geom_smooth(
    method = lm,
    se=FALSE)
