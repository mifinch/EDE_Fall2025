...
```{r}
#split the dataset into early and late years
...
```{r}
EnoDischarge.late_ts <- ts(EnoDischarge.late$discharge.mean, start = c(1985,1), frequency = 365) #must define starting point and freq
```
...
```{r}
?stl
#univariate time series to be decomposed; decomposition must be run for each variable separately 
...
EnoDischarge.late_Components <- mutate(EnoDischarge.late_Components,
        Observed = EnoDischarge.late$discharge.mean, #add observed column     
        Date = EnoDischarge.late$datetime) #add date column

# Visualize how the trend maps onto the data
ggplot(EnoDischarge.late_Components) +
  geom_line(aes(y = Observed, x = Date),  size = 0.25) +
  geom_line(aes(y = trend, x = Date), color = "#c13d75ff") + #add trend line
  geom_hline(yintercept = 0, lty = 2) + #dashed intercept line
  ylab(expression("Discharge (ft"^3*"/s)"))

# Visualize how the seasonal cycle maps onto the data
ggplot(EnoDischarge.late_Components) +
  geom_line(aes(y = Observed, x = Date),  size = 0.25) +
  geom_line(aes(y = seasonal, x = Date), color = "#c13d75ff") + #plot seasonal component
...
```{r}
head(wind_data)
summary(wind_data$AWND) #check NAs

# Adding new column with no missing obs, just for illustration purpose
# In real applications you will simply replace NAs
wind_data_clean <- 
  wind_data %>% 
  mutate( AWND.clean = zoo::na.approx(AWND) ) #create new column AWND.clean, using zoo package, na.approx function to interpolate missing data

#replace using na.approx, rather than omit data (na.omit)
...
f_month <- month(first(wind_data_clean$DATE)) #define first month as first observation on DATE 
f_year <- year(first(wind_data_clean$DATE)) #define first year as first observation

wind_data_ts <- ts(wind_data_clean$AWND.clean,
                   start=c(f_year,f_month),
                   frequency=12) #freq=12; working w/ monthly data
...
# Inspect results
wind_data_trend1
summary(wind_data_trend1) #p<0.05; reject null data is stationary,we have a trend


wind_data_trend2 <- trend::smk.test(wind_data_ts)
# Inspect results
wind_data_trend2
summary(wind_data_trend2) #shows results by each season, overall decrease in trend
...
> Answer: The linear regression has a negative slope, reflecting decrease trend in wind speed over time
