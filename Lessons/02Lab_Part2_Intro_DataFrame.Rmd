...
```{r}
#Option 1
class(car_speed) #numeric value

#Option 2
is.data.frame(car_speed) #FALSE not data frame
```

How could you make it a data frame?

```{r}
df_car_speed <- as.data.frame(car_speed) #turn into df
df_car_speed
class(df_car_speed) #df
```
...
```{r}
car_dist <- cars$dist #save as variable

#Option 1 combine 2 columns
df <- cbind(df_car_speed,car_dist)  #similarly rows could be added using rbind()
class(df)
df #back to original w/ both saved as df
...
class(df_opt2)
df_opt2 #built df from 2 vectors
