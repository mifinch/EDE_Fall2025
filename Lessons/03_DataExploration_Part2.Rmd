...
>Answer: Data exploration is important for recognizing patterns and trends in data. 
...
getwd()
here() #confirms here and getwd are referring to project directory

# 3. Import datasets
USGS.flow.data <- read.csv(
  file = here('Data/Processed/USGS_Site02085000_Flow_Processed.csv'),
  stringsAsFactors = TRUE, # chr to factor
  colClasses = c('site_no' = 'factor') # site no. from int to factor
)
...
#lty = line type
#see Help > data visualization cheat sheet

# color = categorical of approval
ggplot(USGS.flow.data) +
...
# x= gage height mean, y= discharge mean
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

#violin plot, width is proportional to number of values
ggplot(USGS.flow.data) +
...
#date time in x axis is good for time series analysis
```
