...
colnames(NTL.phys.data) #column names
head(NTL.phys.data) #first 6
summary(NTL.phys.data) #summary statistics
str(NTL.phys.data) #structure
dim(NTL.phys.data) #dimensions
...
# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] #filter rows with 0 in column depth
...
NTL.phys.data.JunethruOctober1 <- filter(NTL.phys.data, daynum > 151 & daynum < 305) #used day number equivalent found in dataset
...
```{r arranging data}
NTL.phys.data.depth.ascending <- arrange(NTL.phys.data, depth) #arrange by depth
NTL.phys.data.depth.descending <- arrange(NTL.phys.data, desc(depth)) #specify descending
...
```{r selecting columns}
NTL.phys.data.temps <- select(NTL.phys.data, lakename, sampledate:temperature_C) #create subset with lakename, sampledate through temperature_c
...
```{r mutating data}
#add new column changing celsium to farenheight
...
interval(first(NTL.phys.data.PeterPaul1$sampledate), last(NTL.phys.data.PeterPaul1$sampledate)) #use first and last, recommended approach
```
