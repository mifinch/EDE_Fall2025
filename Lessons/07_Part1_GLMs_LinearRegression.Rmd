...
# Correlation
cor.test(PeterPaul.chem.nutrients$irradianceWater, PeterPaul.chem.nutrients$depth) # -0.555968  - significant negative cor, lower light at greater depth
```
...
par(mfrow = c(2,2), mar=c(4,4,4,4)) #2x2 grid
plot(irradiance.regression)
par(mfrow = c(1,1)) #show all 4 together
```
...
irradiancebydepth <- 
  ...
  ylim(0, 2000) + #remove outlier from scale
  geom_point() 
print(irradiancebydepth) #not linear rxp
```
...
PeterPaul.chem.nutrients <- filter(PeterPaul.chem.nutrients, 
                                   irradianceWater != 0 & irradianceWater < 5000) #filter to eliminate 0s, and remove outliers

irradiance.regression2 <- lm(data = PeterPaul.chem.nutrients, log(irradianceWater) ~ depth) #log of irradiance
summary(irradiance.regression2) #R2 increased to 0.7255
...
irradiancebydepth2 <- 
    ...
    geom_smooth(method = 'lm', se = FALSE, color = "black") #removes CI
print(irradiancebydepth2)
...
PeterPaulnutrients <- 
  ...
  select(tn_ug:po4) %>% #select variables of interest
  na.omit() #remove rows w/ NAs
...
TPAIC <- lm(data = Paul.naomit, tp_ug ~ depth + dissolvedOxygen + 
              temperature_C + tn_ug + po4) #use 5 explanatory variables
