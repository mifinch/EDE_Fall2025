...
# Create subset - Remove columns that are not of interest for analysis
...
```{r joining data}
#combine physical and nutrient data into new dataset using full_join()
...
#more efficient to create a list of 20 files using list.files() function
LitterFiles = list.files(path = "./Data/Raw/NIWO_Litter/", pattern="*.csv", full.names=TRUE)
LitterFiles 

Litter <- LitterFiles %>%
  plyr::ldply(read.csv) #read each element
```
...
#Join trap and litter data into 1 dataset
Trap <- read.csv("./Data/Raw/NEON_NIWO_Litter_trapdata_raw.csv") #24 obs. 23 variables
LitterTrap <- left_join(Litter, Trap, by = c("siteID", "plotID", "trapID")) #1692 obs 39 variables

dim(Litter) #1692   19
dim(Trap) #24 23
dim(LitterTrap) #1692   39
...
NTL.PeterPaul.summaries <- 
  ...
  filter(depth == 0) %>%  #filter where depth is 0
  group_by(lakename, month) %>%  #group by lakename and month
  filter(!is.na(temperature_C) & !is.na(tn_ug) & !is.na(tp_ug)) %>%  #filter na out for temp, tnug, and tpug
