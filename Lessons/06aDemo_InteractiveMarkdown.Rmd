...
#Construct the plot - nitrogen
my_plot_2 <- nutrient_data %>% 
  filter(year4 > start_year & year4 < end_year) %>% #adjust year 
  ggplot(aes(
    x = sampledate,
    y = !!sym(the_parameter), #read the parameter
    color = depth,
    shape = lakename)) +
  geom_point(alpha = 0.4, size = 2) +
  scale_x_date(
...
plot_it('po4', 1995, 2002) #parameter, start, end date
```
...
selectInput(
  ...
  label = 'select the nutrient', #what shows up on the interface
