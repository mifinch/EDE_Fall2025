...
st_bbox(counties_sf) #min and max
...
svi2020_nc_raw <- read.csv(
  ...
  colClasses = c('FIPS' = 'factor')) %>% #bring in as factor
...
#View with mapview - SVI and PM25
mapview(counties_sf_join, 
        zcol = 'E_POV150', #feature colors
        col.regions = brewer.pal(2, 'RdBu')) +  #palatte
  mapview(epa_pm25_sites_sf, cex = 'maxPM')

#view with ggplot
ggplot() + 
  geom_sf(data=counties_sf_join,aes(fill = RPL_THEME1),alpha=0.5) + 
  scale_fill_gradient2(low="red",high="blue",midpoint = 0.5) + #specify color ramp
  geom_sf(data=epa_pm25_sites_sf) +
  labs(fill='Socio\nEcononomic\nIndex') #\n parses title across lines
...
```{r}
#What is a HUC
##hydrologic unit code - defines a drainage area
...
wind_raw <- read.csv(
  ...
  colClasses = c('HUC_12' = 'factor')) %>% #bring in as factor
  mutate(HUC8 = substr(HUC_12, 0,8)) %>%  #extract 1st 8 digits
  group_by(HUC8) %>% 
  summarize(
...
huc8_sf_join <- huc8_sf %>% 
  left_join(wind_raw,by = c("HUC_8" = "HUC8"))
  

#View the outputs - wind energy by HUC8
ggplot() + 
  geom_sf(data=huc8_sf_join, aes(fill = WindEnergy)) + 
  labs(
...
#Map the data - dissolved DWQ_Basin boundaries
#using mapview
mapview(huc8_sf,zcol="DWQ_Basin")
mapview(huc2_sf,zcol="ACRES")

#using ggplot
ggplot() + geom_sf(data=huc8_sf,aes(fill=ACRES)) 
ggplot() + geom_sf(data=huc2_sf,aes(fill=ACRES)) #dissolved
...
#Convert all to UTM Zone 17 (crs = 26917) - find on epsg.io
...
#Start building a map object of Neuse
...
myMap #draws lines between the center points
...
##returns false, true if intersects
...
#Right order shows points and lines
ggplot() +
  geom_sf(data = state_sf_utm, color='red',size=2) +
  geom_sf(data = counties_sf_utm, aes(fill = ALAND), color = 'white')  +
  geom_sf(data = epa_sf_utm, color='blue', size=2) + #point layer added at the end
  scale_fill_gradient(low="yellow", high="darkgreen")
```
